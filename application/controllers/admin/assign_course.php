...
class Assign_course extends CI_Controller {
	...
	function index()
	{
		...
		$teacher_id= $this->courses->get_department_by_assign_course_id(24);
	...
	{		
		...
		try{
			...
			$crud->columns('course_id','assigned_by','year_id','section_id','batch_year','status','department');
			...
			$crud->callback_edit_field('assigned_to',array($this,'get_teachers_dropdown_multiple_edit'));
			...
			$crud->callback_column('department',array($this,'get_department_by_assign_course_id'));
	...
	function call_after_delete($deleted_assign_course_id){
		...
		$this->db->delete('course_assignments', array('assign_course_id' => $deleted_assign_course_id));
	}
	...
	{ 
		...
		if($this->assign->check_duplicate($data,$teacher_id,$assign_course_id)==1){
			...
			$this->form_validation->set_message('check_duplicate','Course Already Assigned. Duplicates not allowed.');
			return false;
		}
		else{
			return true;
	...
	function get_department_by_assign_course_id($value,$row){
		
		return $this->courses->get_department_by_assign_course_id($row->id);
	}
	...
	function get_teachers_dropdown_multiple_edit($value,$row){
	
		return $this->teachers->get_teachers_dropdown_multiple_edit($row);
	}
