...
class Course_assignments extends CI_Controller {
	...
	{		
		...
		try{
			...
			$crud->columns('assign_course_id','topic','lecture_date','created_by','uploaded_file','uploaded_audio','section_id','batch_year','due_date');
			
			$crud->set_rules('due_date','Due Date','callback_check_dates[lecture_date]');
		...
		//	$crud->callback_after_insert(array($this,'call_after_insert'));
			...
			$crud->fields('section_id','batch_year','assign_course_id','lecture_date','due_date','topic','topic_desc','created_by','created_on','uploaded_file','uploaded_audio','refer_links','send_email');
	...
	public function check_dates($due_date)
	{
		$lecture_date=$_POST['lecture_date'];
		
		
		if ($due_date >= $lecture_date)
		{
			return TRUE;
		}
		else
		{
			$this->form_validation->set_message('check_dates', "Due date {$due_date} cannot be less than Assignment date {$lecture_date}");
			return FALSE;
		}
	}
