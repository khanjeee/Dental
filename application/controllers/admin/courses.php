...
class Courses extends CI_Controller {
	...
	{		
		...
		try{
			...
			$crud->set_rules('code', 'Course Code', 'callback_check_duplicate');
	...
	function check_duplicate($value,$row)
	{
		...
		if($this->courses->check_duplicate($course_code,$course_id)==1){
			//dont validate on edit
			$this->form_validation->set_message('check_duplicate','Course Code already Exist');
			return false;
		}
		else{
			return true;
	...
	function call_after_delete($deleted_course_id){
	/*deleting all course data on deletion*/
		$query = $this->db->get_where('assign_course', array('course_id' => $deleted_course_id));
		$result=$query->result();
		if(!empty($result)){
			$assign_course_id= $result[0]->id;
			$this->db->delete('assign_course', array('id' => $assign_course_id));
			$this->db->delete('student_course', array('assign_course_id' => $assign_course_id));
			$this->db->delete('assign_course_teacher', array('assign_course_id' => $assign_course_id));
			$this->db->delete('course_lectures', array('assign_course_id' => $assign_course_id));
			$this->db->delete('course_assignments', array('assign_course_id' => $assign_course_id));
