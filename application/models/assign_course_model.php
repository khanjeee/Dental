 ...
 function __construct()
   ...
   //checks for duplicate entries in db return 1 if exist else 0
    ...
    {	
    	...
    	//check if the duplicate entry found if of own row 
    	if(empty($result) || ($result[0]->id==$assign_course_id)){ //checking course code against own id and if it exists in db or not
    		return 0;
    	}
    	else{
    		return 1;
    	...
    	$result=$query->result();
    	 ...
    	 if(!empty($result)){
    	 	...
    	 	foreach ($result as $key=>$value){
    	 		$dataArr = array('student_id' => $user_student_id,'assign_course_id' => $value->id);
    ...
    function get_assigned_course_dropdownByYearSection($year_id,$section_id, $student_id){
        ...
        $query=$this->db->query("SELECT a.id,c.name
    			FROM assign_course a
    			INNER JOIN courses c ON a.course_id = c.id
    			INNER JOIN  student_course sc on sc.assign_course_id = a.id
    			WHERE a.year_id= ? AND a.section_id = ? AND sc.student_id= ?
    			GROUP BY c.name", array($year_id,$section_id,$student_id));
    ...
    function get_AssignCoursesDDLForTeacher($teacher_id)
    {
        $query = $this->db->query('SELECT ac.id , s.name
                                    FROM assign_course_teacher act
